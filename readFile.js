...
async function readFileWithPromise() {
    try {
        ...
        const data = await fs.readFile("exercise.js", "utf8");
        console.log("File content with promise:", data);
        const filteredData = data.replace(/u/g, "");
        
        await fs.writeFile("output.txt", filteredData);

        console.log("File written successfully with filtered content");
        console.log(filteredData.split('').filter((u)=> u != 'u').join(''));
